...
BUSINESS_CONFIG = {
    'average_surgery_minutes': 110,  # fallback only; overridden by uploaded data when Duration column is present
    'surgeries_per_day': 30,
    'num_ors_per_department': 4,
    'num_departments': 3,
    'work_days_per_month': 22,       # work_days_per_year derived as work_days_per_month × 12
    'revenue_per_surgery': 15000,
    'manual_buffer_minutes': 25,     # Manual MAE: typical error of manual scheduling predictions (minutes)
}
...
def predict_duration():
    ...
    try:
        ...
        # Business impact
        config = get_config()
        # Use avg surgery duration from uploaded data when available; fall back to config
        if has_duration:
            avg_surgery_minutes = float(df_clean['Duration in Minutes'].mean())
        else:
            avg_surgery_minutes = config['average_surgery_minutes']
        model_buffer = round(mae, 1)
        current_buffer = config['manual_buffer_minutes']
        surgeries_per_day = config['surgeries_per_day']
        work_days_per_year = config['work_days_per_month'] * 12

        # Extra surgeries: time freed per surgery × how many surgeries in a day, then / avg duration
        surgeries_gained_daily = (abs(current_buffer) - abs(model_buffer)) * surgeries_per_day / avg_surgery_minutes
        surgeries_gained_monthly_val = surgeries_gained_daily * config['work_days_per_month']
        surgeries_gained_yearly = surgeries_gained_daily * work_days_per_year
        per_or_annual_benefit = surgeries_gained_yearly * config['revenue_per_surgery']
        total_ors = config['num_ors_per_department'] * config['num_departments']
        total_annual_benefit = per_or_annual_benefit * total_ors

        business_impact = {
            'surgeries_gained_daily': float(round(surgeries_gained_daily, 4)),
            'surgeries_gained_monthly': float(round(surgeries_gained_monthly_val, 2)),
            'surgeries_gained_yearly': float(round(surgeries_gained_yearly, 1)),
            'per_or_annual_benefit': int(round(per_or_annual_benefit, 0)),
            'total_ors': total_ors,
            'total_annual_benefit': int(round(total_annual_benefit, 0)),
            'avg_surgery_minutes_used': float(round(avg_surgery_minutes, 1)),
        }
...
def get_config_endpoint():
    ...
    descriptions = {
        ...
        'average_surgery_minutes': 'Average duration of a surgery in minutes (auto-computed from uploaded data)',
        'surgeries_per_day': 'Average number of surgeries performed per OR per day',
        'num_ors_per_department': 'Average number of operating rooms per department',
        'num_departments': 'Number of departments in your facility',
        'work_days_per_month': 'Working days per month — annual figures are derived as this × 12',
        'revenue_per_surgery': 'Average revenue generated per extra surgery ($)',
        'manual_buffer_minutes': 'Typical prediction error of the current manual scheduling method (minutes) — the MAE of surgeon estimates',
    }
...
def reset_config():
    """Reset config to defaults by clearing session overrides."""
